...
- **Interface language:** English, Spanish, or Portuguese for the buttons and labels on this device (Create room, Join, Start/Stop, Type + Send, errors). It is saved in the browser (`mt-ui-lang`) and updates the screen you are already on. It does not change **Spoken**, which still sets the microphone recognizer to `en-US`, `es-ES`, or `pt-BR`. It does not change **Watch**, which still chooses the caption panes on that phone.
...
- **Guest phones (Android or iPhone):** scan **Join on phones** (`/?view=join&room=ABCD`). Before the caption board they answer **What language are you speaking?** and **What language do you want to watch?** (one language or all three), optionally a name, then **Join**. Spoken is for the mic and Type + Send. Watch is only the captions on that phone. They can change both after they are in the room. One speaker at a time. The host can **Reclaim mic**. Chrome on Android is best for live speech. iPhone can always watch and can **Type + Send** if Web Speech fails. Safari often shows the words on that phone without ever marking them final, and can end a heard phrase as no-speech. The phrase is still sent when the mic goes quiet, when that no-speech arrives, or when the utterance ends, so other phones and the TV get one finished caption, not every draft. An empty no-speech stays on screen and is not sent as speech. Safari keeps the first `webkitSpeechRecognition` and only changes `lang` (`es-ES` / `pt-BR`). Choosing Spoken writes that `lang` and the page language before Start, so Spanish and Portuguese are not left on English. Android and desktop may build a new recognizer.
...
Start recognition inside the tap, before the floor claim finishes. Choosing Spoken stamps `es-ES` or `pt-BR` on the first `webkitSpeechRecognition` and on the page language before Start. Safari otherwise keeps English and does not hear Spanish or Portuguese. That first instance is kept; only `lang` changes. If Safari rejects a locale, the phone names it. **Type + Send** does not use Web Speech and still reaches the other phones and the TV.
